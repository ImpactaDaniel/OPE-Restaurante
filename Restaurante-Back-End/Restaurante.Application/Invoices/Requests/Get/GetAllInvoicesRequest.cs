...
{
    public class GetAllInvoicesRequest : InvoiceRequest<GetAllInvoicesRequest>, IRequest<Response<IEnumerable<Invoice>>>
    {
        public int CurrentUserId { get; set; }

        internal class GetAllInvoicesRequestHandler : IRequestHandler<GetAllInvoicesRequest, Response<IEnumerable<Invoice>>>
        {
            ...
            public async Task<Response<IEnumerable<Invoice>>> Handle(GetAllInvoicesRequest request, CancellationToken cancellationToken)
            {
                try
                {
                    ...
                    return new Response<IEnumerable<Invoice>>(true, await _invoiceRespository.GetAll(cancellationToken));

                }
                catch (RestauranteException e)
                {
                    _notifier.AddNotification(NotificationHelper.FromException(e));
                    return new Response<IEnumerable<Invoice>>(false, null);
                }
                catch (Exception e)
                {
